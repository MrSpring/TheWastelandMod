...
public class Rectangle {
   ...
   public static boolean checkConflict(Rectangle rectA, Rectangle rectB, int minDist) {
      return rectA.position.X <= rectB.position.X + rectB.width + minDist && rectA.position.X + rectA.width >= rectB.position.X - minDist && rectA.position.Z + rectA.length >= rectB.position.Z - minDist && rectA.position.Z <= rectB.position.Z + rectB.length + minDist;
   }
