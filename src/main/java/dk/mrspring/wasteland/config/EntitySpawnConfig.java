...
public class EntitySpawnConfig {
   ...
   public static List wastelandCreatures;
   public static List forestCreatures;
   public static List mountainsCreatures;
   public static List cityCreatures;
   ...
   private static List copyArrayList(List list) {
   ...
   private static List getAllBiomeEntities() {
      ...
      for(int creatures = 0; creatures < type.length; ++creatures) {
         ...
         for(int j = 0; j < biomes.length; ++j) {
            ...
            for(k = 0; k < monsterCreatures.size(); ++k) {
               if(creatureIndex((SpawnListEntry)monsterCreatures.get(k), allMonsterCreatures) == -1) {
                  allMonsterCreatures.add(copy((SpawnListEntry)monsterCreatures.get(k)));
               }
            }

            for(k = 0; k < passiveCreatures.size(); ++k) {
               if(creatureIndex((SpawnListEntry)passiveCreatures.get(k), allPassiveCreatures) == -1) {
                  allPassiveCreatures.add(copy((SpawnListEntry)passiveCreatures.get(k)));
               }
            }

            for(k = 0; k < waterCreatures.size(); ++k) {
               if(creatureIndex((SpawnListEntry)waterCreatures.get(k), allWaterCreatures) == -1) {
   ...
   private static void addRemainingCreatures(List creatures) {
   ...
   private static int creatureIndex(SpawnListEntry entry, List list) {
      return creatureIndex(entry.entityClass, list);
   }

   private static int creatureIndex(Class entry, List list) {
      for(int i = 0; i < list.size(); ++i) {
