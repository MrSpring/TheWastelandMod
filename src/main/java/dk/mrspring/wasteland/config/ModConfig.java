...
//        System.out.println(block == null);
    ...
    {
        String[] modidAndName = string.split(":");
        return modidAndName.length == 3 ? Integer.parseInt(modidAndName[2]) : 0;
    }
    ...
    {
        return getBlockMetaFromString(surfaceBlockString);
    }
    ...
    {
        return getBlockMetaFromString(lakeLiquidString);
    }
    ...
    public static String getSpawnerCreature(Random random)
    {
        ...
        int creatureType;
        for (creatureType = 0; creatureType < num; ++creatureType)
        {
            totalChance += spawnChance[creatureType];
        }

        creatureType = random.nextInt(totalChance);
        totalChance = 0;

        for (int i = 0; i < num; ++i)
        {
            if (creatureType < totalChance + spawnChance[i])
            {
                return mobName[i];
            }
    ...
    public static int[] getWoodType(Random random)
    {
        ...
        int woodType;
        for (woodType = 0; woodType < num; ++woodType)
        {
            totalChance += woodTypes[2][woodType];
        }

        woodType = random.nextInt(totalChance);
        totalChance = 0;

        for (int i = 0; i < num; ++i)
        {
            if (woodType < totalChance + woodTypes[2][i])
            {
                out[0] = woodTypes[0][i];
                out[1] = woodTypes[1][i];
                return out;
