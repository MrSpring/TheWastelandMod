...
public class ChunkProviderWasteland implements IChunkProvider
{
    ...
    private Block field_177476_s = Blocks.air;
    ...
    public void populate(IChunkProvider provider, int par2, int par3)
    {
        ...
        this.rand.setSeed((long) par2 * k + (long) par3 * l ^ this.worldObj.getSeed());
        boolean flag = false;
        ChunkCoordIntPair chunkcoordintpair = new ChunkCoordIntPair(par2, par3);

        MinecraftForge.EVENT_BUS.post(new Pre(provider, worldObj, rand, par2, par3, flag));
        ...
        boolean doGen = TerrainGen.populate(provider, worldObj, rand, par2, par3, flag, PopulateChunkEvent.Populate.EventType.DUNGEON);
        ...
        if (TerrainGen.populate(provider, worldObj, rand, par2, par3, flag, PopulateChunkEvent.Populate.EventType.ANIMALS))
        {
            SpawnerAnimals.performWorldGenSpawning(this.worldObj, biomegenbase, i + 8, j + 8, 16, 16, this.rand);
        }
        blockpos = blockpos.add(8, 0, 8);

        doGen = net.minecraftforge.event.terraingen.TerrainGen.populate(provider, worldObj, rand, par2, par3, flag, PopulateChunkEvent.Populate.EventType.ICE);
        ...
        EVENT_BUS.post(new net.minecraftforge.event.terraingen.PopulateChunkEvent.Post(provider, worldObj, rand, par2, par3, flag));
